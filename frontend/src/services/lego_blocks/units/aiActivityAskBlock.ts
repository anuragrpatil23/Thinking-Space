...
// An "ask" is a record from the OLD organizer — a question, idea, missed idea,
// or company-to-study that Anurag wrote by hand, mostly before the chains
// begin (2026-05-18). Asks are the forward half of the loop; undertakings are
// the backward half. They are never migrated into undertakings (that would
// give them permanently empty tails and destroy the fact that they are still
// open). They are read here and joined to undertakings by seam edges.
//
...
export interface Ask {
  ...
  category: string
  /** ISO date the ask was opened (`created_at`). */
  openedDate: string
}
...
/** Category code embedded in an ask key (`f9-qt-e-318` → `QT`), or '' if the
 *  key doesn't match the old organizer's `<project>-<CODE>-E-<n>` shape. */
export function askCategoryCodeBlock(key: string): string {
  const m = /^[a-z0-9]+-([a-z]+)-e-\d+/i.exec(key)
  return m ? m[1].toUpperCase() : ''
}

export function askCategoryLabelBlock(code: string): string {
...
/** Parse one old-organizer epic file into an Ask. Null when it isn't an epic
 *  or has no key — hand-editable files must not take the reader down. */
export function parseAskMarkdownBlock(content: string): Ask | null {
  ...
  if (!key) return null
  // Only epics are asks; programs/thoughts/idea_buckets are structure.
  if (asString(parsed.record_kind) && asString(parsed.record_kind) !== 'epic') return null

  const code = askCategoryCodeBlock(key)
  return {
    key,
    title: asString(parsed.title) || key,
    categoryCode: code,
    category: askCategoryLabelBlock(code),
