...
| Derivation | AI-activity strata are re-derivable from raw sessions; never file a record under a derived address or a position; a hash over inputs cannot see a code change (name the lever); absence is not evidence; derived layers fail loudly; hooks capture, the app derives; authorship changes only when someone speaks — automation never holds a sitting open; an automatic run may create a derived record but only a human may replace one. | [docs/contracts/DERIVATION.md](docs/contracts/DERIVATION.md) |
