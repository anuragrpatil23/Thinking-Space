...
14. **Editor = one CM6 engine, decorations on top (locked 2026-07-17)**: no ProseMirror/Notion block JSON — markdown+YAML stay byte-identical on disk; richer editing is CM6 decorations (Live Preview model) in `MarkdownRichEditorBlock` (units: markdownInlineImageExtensionBlock, markdownSyntaxHidingExtensionBlock, markdownTaskCheckboxExtensionBlock, editorLanguageBlock for per-file-type grammar routing; markdown decorations only mount for markdown files). User toggle `livePreviewSyntaxHiding`; with it on, click-to-edit replaces the view-mode pencil for text docs.
