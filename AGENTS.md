...
18. **Cross-device settings roaming (locked 2026-07-19)**: the mechanism is `.thinking-space/preferences/ui.json` via `vaultUiPreferencesOrch` — extend `VaultUiPreferencesBlock`, never invent parallel settings files. AI-activity display prefs roam through its `aiActivityPrefs` group (startup pull → localStorage via public setters; debounced write-through push via storageKeyBlock's write listener). User profile roams via `.thinking-space/profile.json`; iCloud guards: `.NAME.icloud` placeholders count as existing in `CapacitorVaultFS.exists`, and a pure-default profile is never seed-written over the vault. Device-bound settings (provider endpoints, Claude-CLI-only options, vault paths, nav rail layout) never roam.
