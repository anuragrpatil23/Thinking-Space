...
- **Known-open: ~750 MB of the idle footprint is the AI-activity subsystem** (narrowed 2026-07-25 by cold-launch traces on iPad Pro M1; force-relaunch mid-trace with `devicectl device process launch --terminate-existing`). Floors: normal 1078 MB (peak 1728) / AI-activity OFF 301–310 MB (peak ~780). True baseline is ~300 MB and **vault sync costs ~0 retained** despite reading 352 MB — it is the control proving read volume alone leaves no floor. Memory is set within ~13s of launch at 60–155% CPU then sits flat forever; an hours-old process hit 2224 MB. Method: temporary module-level kill-switches → `--dirty` ship → cold-launch trace → diff the floor. Activity Monitor suffices — do NOT enable `webContentsDebuggingEnabled` (Release WKWebViews are non-inspectable; it widens the trust boundary) and do NOT try `Allocations --attach` on `WebContent` (system extension, no `get-task-allow`, hard block). Eliminated — do not retry: Dexie records (`searchText` uses a 200-char excerpt), JSC substring pinning via `topic`/`touchedPaths` (a `JSON.parse(JSON.stringify())` flatten made it *worse*: 1078 → 1326 MB), and per-launch transcript re-parsing (mtime cache is exact, 2,859/2,859 match; `ai-activity-cache.json` is 3 MB for 5,204 sessions). Prime remaining suspect is the AI-activity **rendering** path, since disabling the load also unmounts that UI — separate the two next.
