...
17. **iOS native chrome (locked 2026-07-19)**: one native SwiftUI shell for iPhone AND iPad. iPhone = collapsing Safari-style bottom dock; iPad = single-row 48pt top bar with a Files-app centered tab pill (sidebar toggle inside the pill in portrait, separate glass button in landscape; `+`/tab-grid folded into the pill; tab switcher is an anchored popover, not the phone sheet; sync lives in the ⋯ menu). Bar height is coupled: `nativeChromePadBarHeight` (TopChromeView.swift) ↔ `48px` clearance rules (index.css) ↔ PhoneShellView veil depth. Fullscreen web focus modes (Excalidraw) hide the chrome via immersion leases (`nativeChromeImmersionBlock` / `useNativeChromeImmersionBlock`); the hidden veil must not hit-test. Native owns the left-edge swipe (web handler stands down under native chrome; the Swift edge recognizer needs a simultaneous-recognition delegate to win over WKWebView). Overscroll model: content bounces, viewport-pinned backdrops stay fixed (phone AND tablet rules); canvas surfaces (vault graph) set `touch-action: none`. Full details: CLAUDE.md "iOS Native Chrome Decisions".
18. **Vault-sync indicator = the refresh pill (locked 2026-07-19)**: sync never gets its own surface (banner and top-edge hairline both rejected). `backgroundActivityBlock` channels: syncs publish `ambient`, the banner renders only `alert` (escalation). `useAmbientSyncActivityBlock` gates visibility behind a 500ms threshold and feeds both `SyncRefreshButtonBlock` (Electron: icon-only pill, spinning icon inside a small accent progress circle, determinate syncs widen it with a `completed/total` count; click stays cheap UI refresh, not sync) and the iOS bridge (`syncActive`/`syncCompleted`/`syncTotal` → `SyncSpinIconView`: iPad refresh button, iPhone bottom pill). Full details: CLAUDE.md "Vault-Sync Indicator".
19. **Cross-device settings roaming (locked 2026-07-19)**: the mechanism is `.thinking-space/preferences/ui.json` via `vaultUiPreferencesOrch` — extend `VaultUiPreferencesBlock`, never invent parallel settings files. AI-activity display prefs roam through its `aiActivityPrefs` group (startup pull → localStorage via public setters; debounced write-through push via storageKeyBlock's write listener). User profile roams via `.thinking-space/profile.json`; iCloud guards: `.NAME.icloud` placeholders count as existing in `CapacitorVaultFS.exists`, and a pure-default profile is never seed-written over the vault. Device-bound settings (provider endpoints, Claude-CLI-only options, vault paths, nav rail layout) never roam.
...
- **Compare file mtimes at whole-second resolution, never `===` on the raw float** (root-caused + fixed 2026-07-25; `sameMtimeBlock` in `aiActivityCacheBlock.ts`, mirroring `normalizeEpochSeconds` in `vaultSyncOrch`). Adapters emit *fractional* seconds from different sources (Electron `mtimeMs/1000`, iOS `timeIntervalSince1970`, Capacitor FS) and caches round-trip them through JSON, so exact equality reports "changed" for untouched files — 294/2,859 mismatches in the last ULP on Mac, effectively 100% on iOS. That one `===` re-read and re-parsed all ~1,500 vault transcripts (~524 MB) every launch: idle floor **1078 → 313 MB (−71%)**, startup peak **1728 → 793 MB (−54%)**, and the 13s 60–155% CPU launch burn largely gone. Floor to seconds in any new mtime check.
- iOS memory method (reuse this): temporary module-level kill-switches → `--dirty` ship → cold-launch trace (`devicectl device process launch --terminate-existing`) → diff the floor. Compare **foreground** plateaus only — a drop alongside CPU → 0.0 is backgrounding, not a fix. Activity Monitor suffices; do NOT enable `webContentsDebuggingEnabled` (Release WKWebViews are non-inspectable; widens the trust boundary) and do NOT try `Allocations --attach` on `WebContent` (system extension, no `get-task-allow`, hard block). Eliminated — do not retry: Dexie records (`searchText` uses a 200-char excerpt), JSC substring pinning via `topic`/`touchedPaths` (a `JSON.parse(JSON.stringify())` flatten made it *worse*: 1078 → 1326 MB), and the AI-activity **rendering** path (gating render with the load on moved the floor ~0). **Vault sync costs ~0 retained** despite reading 352 MB — the control proving read volume alone leaves no floor. Never truncate when testing an equality production performs exactly; doing so once "proved" this cache exact and hid the bug for an hour.
